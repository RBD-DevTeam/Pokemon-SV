...
static void BagListMenuItemPrintFunc(u8 windowId, u32 itemId, u8 y)
{
    ...
    if (itemId != -2 && sBagMenuDisplay->nItems[gBagMenuState.pocket] != itemId)
    {
        ...
        if (gBagMenuState.pocket != 2 - 1 && !itemid_is_unique(bagItemId))
        {
...
static void bag_menu_print_cursor(u8 y, u8 colorIdx)
{
    if (colorIdx == 0xFF)
    {
        FillWindowPixelRect(0, ((0) | ((0) << 4)), 1, y, GetMenuCursorDimensionByFont(FONT_2, 0), GetMenuCursorDimensionByFont(FONT_2, 1));
    }
    else
    {
...
static void Task_ItemContext_FieldGive(u8 taskId)
{
    ...
    else if (gBagMenuState.pocket != 2 - 1 && !itemid_is_unique(itemId))
    {
        Bag_BeginCloseWin0Animation();
        gTasks[taskId].func = ItemMenu_StartFadeToExitCallback;
    }
...
static void GoToTMCase_Give(void)
{
    InitTMCase(TMCASE_FROMPARTYGIVE, ReturnToBagMenuFromSubmenu_Give, 0);
}
...
static void GoToTMCase_PCBox(void)
{
    InitTMCase(TMCASE_FROMPOKEMONSTORAGEPC, ReturnToBagMenuFromSubmenu_PCBox, 0);
}
...
static void GoToTMCase_Sell(void)
{
    InitTMCase(TMCASE_FROMMARTSELL, ReturnToBagMenuFromSubmenu_Sell, 0);
}
