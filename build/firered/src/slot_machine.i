...
bool8 (*const sSlotMachineSetupTasks[])(u8 *, struct SlotMachineSetupTaskData *) = {
    [0] = SlotsTask_GraphicsInit,
    [1] = SlotsTask_FadeOut,
    [2] = SlotsTask_UpdateLineStates,
    [3] = SlotsTask_ClefairyUpdateOnReelsStart,
    [4] = SlotsTask_StartClefairyDanceAndWinningLineFlash,
    [5] = SlotsTask_StopWinningLineFlashTask,
    [6] = SlotsTask_ClefairyFainted,
    [7] = SlotsTask_ClefairyNeutral,
    [8] = SlotsTask_UpdateCoinsDisplay,
    [9] = SlotsTask_MessageOutOfCoins,
    [10] = SlotsTask_AskQuitPlaying,
    [11] = SlotsTask_DestroyYesNoMenu,
    [12] = SlotsTask_PressReelButton,
    [13] = SlotsTask_ReleaseReelButtons,
    [14] = SlotsTask_ShowHelp,
    [15] = SlotsTask_HideHelp
};
...
static void StopReel2(u16 whichReel)
{
    ...
    if (numPossiblePositions == 0)
    {
        ...
        if (sSlotMachineState->machineBias == 5 || sSlotMachineState->machineBias == 6)
            pos = 4;
        else
            pos = 0;
    }
...
static bool32 TestReelIconAttribute(s32 attr, s32 icon)
{
    ...
    case 2:
        return icon == 4 ? 1 : 0;
    case 3:
        return icon == 5 || icon == 6 ? 1 : 0;
    case 4:
        return icon == 2 || icon == 3 ? 1 : 0;
    case 5:
        return icon == 1 ? 1 : 0;
    case 6:
        return icon == 0 ? 1 : 0;
...
static void CalcSlotBias(void)
{
    ...
    if (sSlotMachineState->machineBias < 5)
    {
        if (sSlotMachineState->biasCooldown == 0)
        {
            if ((Random() & 0x3FFF) < biasChances[6])
...
static u16 CalcPayout(void)
{
    ...
    for (i = 0; i < 5; i++)
    {
        if (sSlotMachineState->bet >= sRowAttributes[i][3])
        {
            int curMatch;
            if (TestReelIconAttribute(1, visibleIcons[sRowAttributes[i][0]]))
                curMatch = TestReelIconAttribute(2, visibleIcons[sRowAttributes[i][1]]) ? 2 : 1;
            else if (visibleIcons[sRowAttributes[i][0]] == visibleIcons[sRowAttributes[i][1]] && visibleIcons[sRowAttributes[i][0]] == visibleIcons[sRowAttributes[i][2]])
                curMatch = ReelIconToPayoutRank(visibleIcons[sRowAttributes[i][0]]);
            else
                curMatch = 0;
            if (curMatch != 0)
            {
...
static void Slot_PrintOnWindow0(const u8 * str)
{
    ...
    AddTextPrinterParameterized5(0, FONT_2, str, 1, 2, 0xFF, ((void *)0), 1, 2);
}
...
static void Task_FlashWinningLine(u8 taskId)
{
    ...
    case 1:
        ...
        if (data[3] == 0)
        {
            ...
            BlendPalettes(0x00000040, data[5], ((0) | ((0) << 5) | ((0) << 10)));
        }
        else
        {
            data[4]++;
            if (data[4] > 1)
            {
                ...
                data[5]++;
                data[5] &= 1;
                BlendPalettes(0x00000040, data[5] * 8, ((0) | ((0) << 5) | ((0) << 10)));
            }
        }
...
static void PressReelButton(u8 reel, u8 taskId)
{
    ...
    SetSlotMachineSetupTask(12, taskId);
}
