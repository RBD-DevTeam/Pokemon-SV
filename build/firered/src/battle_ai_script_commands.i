    ...
    if (GetBattlerSide(battlerId) == 0)
    {
        ...
        if (gBaseStats[gBattleMons[battlerId].species].abilities[0] != 0)
        {
            if (gBaseStats[gBattleMons[battlerId].species].abilities[1] != 0)
            {
                ...
                if (Random() % 2)
                    (gBattleResources->ai)->funcResult = gBaseStats[gBattleMons[battlerId].species].abilities[0];
                else
                    (gBattleResources->ai)->funcResult = gBaseStats[gBattleMons[battlerId].species].abilities[1];
            }
            else
            {
                (gBattleResources->ai)->funcResult = gBaseStats[gBattleMons[battlerId].species].abilities[0];
