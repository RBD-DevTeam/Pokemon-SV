...
{
    u32 x;
    DrawStdFrameWithCustomTileAndPalette(2, 0, 0x081, 0x0C);
    x = 0x40 - GetStringWidth(FONT_0, gText_DepositItem, 0);
    AddTextPrinterParameterized(2, FONT_0, gText_DepositItem, x / 2, 1, 0, ((void *)0));
}
...
u8 ShowBagWindow(u8 whichWindow, u8 nItems)
{
    if (sOpenWindows[whichWindow] == 0xFF)
    {
        ...
        if (whichWindow != 6)
        {
            DrawStdFrameWithCustomTileAndPalette(sOpenWindows[whichWindow], 0, 0x064, 0xE);
        }
        else
        {
            DrawStdFrameWithCustomTileAndPalette(sOpenWindows[whichWindow], 0, 0x081, 0xC);
        }
        ScheduleBgCopyTilemapToVram(0);
    }
