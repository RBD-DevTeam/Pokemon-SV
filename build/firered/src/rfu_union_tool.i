...
# 9 "src/rfu_union_tool.c" 2
# 1 "include/constants/event_object_movement.h" 1
# 10 "src/rfu_union_tool.c" 2
# 1 "include/constants/union_room.h" 1
# 11 "src/rfu_union_tool.c" 2
# 1 "include/constants/event_objects.h" 1
# 12 "src/rfu_union_tool.c" 2

static __attribute__((section("ewram_data"))) struct UnionObj * UnionObjWork = ((void *)0);
...
u8 ZeroUnionObjWork(struct UnionObj * ptr)
{
    ...
    UnionObjWork = ptr;
    (UnionObjWork != ((void *)0)) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "rfu_union_tool.c"), 442, "UnionObjWork != NULL", 1);
    for (i = 0; i < 8; i++)
    {
        ptr[i].state = 0;
        ptr[i].gfxId = 0;
...
static bool32 SpawnGroupLeader(u32 playerIdx, u32 gender, u32 idMod256)
{
    ...
    (UnionObjWork != ((void *)0)) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "rfu_union_tool.c"), 561, "UnionObjWork != NULL", 1);
    ptr->schedAnim = 1;
...
{
    struct UnionObj * ptr = &UnionObjWork[playerIdx];
    (UnionObjWork != ((void *)0)) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "rfu_union_tool.c"), 577, "UnionObjWork != NULL", 1);
    ptr->schedAnim = 2;
    if (ptr->state == 1)
    {
        return 1;
...
{
    s32 i;
    (UnionObjWork != ((void *)0)) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "rfu_union_tool.c"), 643, "UnionObjWork != NULL", 1);
    for (i = 0; i < 8; i++)
    {
...
static u8 StartUnionObjAnimTask(void)
{
    if (FuncIsActiveTask(Task_AnimateUnionObjs) == 1)
    {
...
void CreateGroupMemberObjectsInvisible(u8 * sprite_ids, s32 group)
{
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        sprite_ids[obj_id] = CreateVirtualObject(25, obj_id - 0x38, sUnionPartnerCoords[group][0] + sFacingDirectionOffsets[i][0], sUnionPartnerCoords[group][1] + sFacingDirectionOffsets[i][1], 3, 1);
        SetVirtualObjectInvisibility(obj_id - 0x38, 1);
    }
}
...
static void SpawnGroupMember(u32 groupNo, u32 memberNo, u8 direction, struct GFtgtGname * gname)
{
    ...
    if (RfuUnionGroupMemberIsInvisible(groupNo, memberNo) == 1)
    {
        SetVirtualObjectInvisibility(objId - 0x38, 0);
        SetVirtualObjectSpriteAnim(objId - 0x38, 1);
    }
...
{
    s32 x, y;
    SetVirtualObjectSpriteAnim(5 * group + member - 0x38, 2);
...
static void AssembleGroup(u32 group, struct GFtgtGname * gname)
{
    ...
    if (IsVirtualObjectInvisible(5 * group - 0x38) == 1)
    {
        if (IsUnionRoomPlayerFacingTileAt(group, 0, x, y) == 1 || IsUnionRoomPlayerFacingTileAt(group, 0, x2, y2) == 1)
        {
            return;
        }
    ...
    for (i = 1; i < 5; i++)
    {
        ...
        else if (IsUnionRoomPlayerFacingTileAt(group, i, x, y) == 0 && IsUnionRoomPlayerFacingTileAt(group, i, x2, y2) == 0)
        {
...
static void UpdateUnionRoomPlayerSprites(struct UnkStruct_URoom * groups)
{
    ...
    {
        if (x20_p[i].groupScheduledAnim == 1)
        {
            SpawnGroupLeaderAndMembers(i, &x20_p[i].gname_uname.gname);
        }
        else if (x20_p[i].groupScheduledAnim == 2)
        {
...
bool32 RfuUnionTool_GetGroupAndMemberInFrontOfPlayer(struct UnkStruct_Main0 *main0_p, s16 *member_p, s16 *group_p, u8 *spriteIds)
{
    ...
    for (i = 0, x20_p = main0_p->arr; i < 8; i++)
    {
        for (j = 0; j < 5; j++)
        {
            ...
            if (IsVirtualObjectInvisible(objId - 0x38) != 0)
            {
                continue;
            }
            if (IsVirtualObjectAnimating(objId - 0x38) != 0)
            {
                continue;
            }
            if (x20_p[i].groupScheduledAnim != 1)
