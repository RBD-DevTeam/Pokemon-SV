...
void PSA_PrintMessage(u8 messageId)
{
    ...
    AddTextPrinterParameterized5(0, FONT_2, scene->textBuf, strWidth, 0, textSpeed, ((void *)0), 0, 4);
}
...
void DrawLevelUpWindowPg1(u16 windowId, u16 *beforeStats, u16 *afterStats, u8 bgColor, u8 fgColor, u8 shadowColor)
{
    ...
    for (i = 0; i < 6; i++)
    {
        AddTextPrinterParameterized3(windowId, FONT_2, 0, i * 15, textColor, 0xFF, sLevelUpWindowStatNames[i]);
        StringCopy(textbuf, diffStats[i] >= 0 ? gText_LevelUp_Plus : gText_LevelUp_Minus);
        AddTextPrinterParameterized3(windowId, FONT_2, 56, i * 15, textColor, 0xFF, textbuf);
        textbuf[0] = 0x00;
        x = abs(diffStats[i]) < 10 ? 12 : 6;
        ConvertIntToDecimalStringN(textbuf + 1, abs(diffStats[i]), STR_CONV_MODE_LEFT_ALIGN, 2);
        AddTextPrinterParameterized3(windowId, FONT_2, x + 56, i * 15, textColor, 0xFF, textbuf);
    }
}
...
void DrawLevelUpWindowPg2(u16 windowId, u16 *currStats, u8 bgColor, u8 fgColor, u8 shadowColor)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        x = 6 * (4 - ndigits);
        AddTextPrinterParameterized3(windowId, FONT_2, 0, i * 15, textColor, 0xFF, sLevelUpWindowStatNames[i]);
        AddTextPrinterParameterized3(windowId, FONT_2, 56 + x, i * 15, textColor, 0xFF, textbuf);
    }
}
