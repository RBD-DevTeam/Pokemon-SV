...
static void BuyMenuPrintPriceInList(u8 windowId, u32 item, u8 y)
{
    ...
    if (item != -2)
    {
        ...
        BuyMenuPrint(windowId, FONT_0, gStringVar4, 0x69, y, 0, 0, 0xFF, 1);
    }
}
...
static void Task_BuyHowManyDialogueInit(u8 taskId)
{
    ...
    ScheduleBgCopyTilemapToVram(0);
    maxQuantity = GetMoney(&gSaveBlock1Ptr->money) / itemid_get_market_price(data[5]);
    if (maxQuantity > 99)
...
static void Task_BuyHowManyDialogueHandleInput(u8 taskId)
{
    ...
    {
        gShopData.itemPrice = itemid_get_market_price(data[5]) * data[1];
        BuyMenuPrintItemQuantityAndPrice(taskId);
    }
    else
    {
