...
void ApplyGlobalTintToPaletteSlot(u8 slot, u8 count)
{
    ...
    CpuFastSet(gPlttBufferUnfaded + slot * 16, gPlttBufferFaded + slot * 16, ((count * 16 * sizeof(u16))/(32/8) & 0x1FFFFF));
}
...
{
    LoadTilesetPalette(mapLayout->primaryTileset, 0, 7 * 16 * 2);
}

void LoadSecondaryTilesetPalette(const struct MapLayout *mapLayout)
{
    LoadTilesetPalette(mapLayout->secondaryTileset, 7 * 16, (13 - 7) * 16 * 2);
}
