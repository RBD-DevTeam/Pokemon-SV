...
static const u8 sUnionRoomObjGfxIds[2][NUM_UNION_ROOM_CLASSES + 2] = {
    [0] = {
        41,
        54,
        39,
...
void CreateUnionRoomPlayerSprites(u8 * spriteIds, s32 leaderId)
{
    ...
    for (memberId = 0; memberId < 5; memberId++)
    {
        ...
        spriteIds[id] = CreateVirtualObject(25,
                                                 ...
                                                 3, 1);
        SetVirtualObjectInvisibility(id - (64 - MAX_UNION_ROOM_LEADERS), 1);
    }
}
...
static u8 GetNewFacingDirectionForUnionRoomPlayer(u32 memberId, u32 leaderId, struct RfuGameData * gameData)
{
    ...
    else if (gameData->activity == (5 | 0x40))
        return 1;
    else
        return 4;
}
...
{
    return IsVirtualObjectInvisible((5 * leaderId + memberId) - (64 - MAX_UNION_ROOM_LEADERS));
}
...
static void SpawnGroupMember(u32 leaderId, u32 memberId, u8 graphicsId, struct RfuGameData * gameData)
{
    ...
    {
        SetVirtualObjectInvisibility(id - (64 - MAX_UNION_ROOM_LEADERS), 0);
...
{
    s32 x, y;
    SetVirtualObjectSpriteAnim((5 * leaderId + memberId) - (64 - MAX_UNION_ROOM_LEADERS), 2);
    GetUnionRoomPlayerCoords(leaderId, memberId, &x, &y);
    MapGridSetMetatileImpassabilityAt(x, y, 0);
}
...
static void AssembleGroup(u32 leaderId, struct RfuGameData * gameData)
{
    ...
    if (IsVirtualObjectInvisible((5 * leaderId + 0) - (64 - MAX_UNION_ROOM_LEADERS)) == 1)
    {
...
static void SpawnGroupLeaderAndMembers(u32 leaderId, struct RfuGameData * gameData)
{
    ...
    case 20 | 0x40:
        ...
        for (i = 0; i < 5; i++)
            DespawnGroupMember(leaderId, i);
        break;
    case ACTIVITY_BATTLE_SINGLE | 0x40:
    case 4 | 0x40:
    case 5 | 0x40:
    case 8 | 0x40:
    case 17 | 0x40:
...
bool32 TryInteractWithUnionRoomMember(struct RfuPlayerList *list, s16 *memberIdPtr, s16 *leaderIdPtr, u8 *spriteIds)
{
    ...
    for (leaderId = 0, leaders = list->players; leaderId < MAX_UNION_ROOM_LEADERS; leaderId++)
    {
        for (memberId = 0; memberId < 5; memberId++)
        {
            ...
            if (IsVirtualObjectInvisible(objId - (64 - MAX_UNION_ROOM_LEADERS)) != 0)
                continue;
            if (IsVirtualObjectAnimating(objId - (64 - MAX_UNION_ROOM_LEADERS)) != 0)
                continue;
...
{
    TurnVirtualObject(5 * leaderId - (64 - MAX_UNION_ROOM_LEADERS) + memberId, direction);
}
