...
static void CreateNormalContextMenu(u8 taskId)
{
    ...
    else if (MenuHelpers_LinkSomething() == 1 || InUnionRoom() == 1)
    {
...
{
    s16 * data = gTasks[taskId].data;
    ConvertIntToDecimalStringN(gStringVar3, itemid_get_market_price(BagGetItemIdByPocketPosition(5, data[1])) / 2 * data[8], STR_CONV_MODE_LEFT_ALIGN, 6);
...
static void Task_Sell_PrintSelectMultipleUI(u8 taskId)
{
    ...
    SellMultiple_UpdateSellPriceDisplay(itemid_get_market_price(BagGetItemIdByPocketPosition(5, data[1])) / 2 * data[8]);
    PrintMoneyInWin2();
...
static u8 GetOrCreateVariableWindow(u8 winIdx)
{
    ...
    if (retval == 0xFF)
    {
        ...
        if (winIdx == 2 || winIdx == 6 || winIdx == 7 || winIdx == 8 || winIdx == 9)
            DrawStdFrameWithCustomTileAndPalette(sVariableWindowIds[winIdx], 0, 0x00A, 0xC);
        else
            DrawStdFrameWithCustomTileAndPalette(sVariableWindowIds[winIdx], 0, 0x001, 0xE);
        ScheduleBgCopyTilemapToVram(2);
